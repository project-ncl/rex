...
public class InvokeStartTask extends TransactionalControllerTask{
    ...
    private static final Logger logger = LoggerFactory.getLogger(InvokeStartTask.class);


    public InvokeStartTask(TransactionManager tm, Service service, ServiceControllerImpl controller) {
        super(tm);
        ...
        this.threadContext = CDI.current().select(ThreadContext.class).get();
